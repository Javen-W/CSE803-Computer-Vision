...
def p7(M):
    ...
    return M / np.sqrt((M ** 2).sum(axis=1, keepdims=True))
...
def p10(L):
    ...
    M = len(L)
    centroids = [m.mean(axis=0, keepdims=False) for m in L]
    dists = np.zeros((M, M))
    for i in range(M):
        dists[i, :] = np.sqrt(np.sum((centroids[i] - centroids) ** 2, axis=1))
    return dists
